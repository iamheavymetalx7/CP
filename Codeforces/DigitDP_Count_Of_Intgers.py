...
def solve():
    ...
    # print(num1,num2,min_sum,max_sum)
    maxi =0

    def recur(tight,idx,curr,s):
        # print(tight,idx,curr,s)
        nonlocal maxi
        
        if idx==len(s):
            # print(curr,"isnsideeee",min_sum<=curr<=max_sum)
            maxi = max(maxi,curr)
            return min_sum<=curr<=max_sum
        

        ub  = 9 if not tight else int(s[idx])
        # print(ub,"hehehehe ub")
        ans = 0
        ...
        return ans
    # print(maxi,"dafuqqq")
    ans = recur(True,0,0,num2)

    ans2 = recur(True,0,0,num1)
