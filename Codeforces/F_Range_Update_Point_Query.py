...
'''
Let 𝑆(𝑛)denote the sum of the digits of 𝑛. The key observation is the following:
after the operation is applied to index 𝑖 thrice, it won't change 
after any further operations. The proof† is provided at the bottom of the editorial.

So we only need to update 𝑎𝑖 if it's been updated at most 2
times so far; otherwise, we can ignore it. This allows us to do the 
following solution: store the current "active" indices (that is, 
indices that have been updated ≤2 times) in a sorted list ,
(for example, set in C++). Then:

1 𝑙 𝑟 — search for the smallest active index at least 𝑙
(since the list is sorted, we can do it in (log𝑛)). Afterwards, update that index
(replace 𝑎𝑖 with 𝑆(𝑎𝑖)), remove it if it's no longer active, and binary search 
for the next largest active index in the sorted list, until we pass 𝑟.
