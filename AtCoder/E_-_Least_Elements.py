#https://atcoder.jp/contests/abc281/submissions/37180444
#Upsolving
#Atcoder_Assemble
import math
import collections
...
def solve():
    ...
    arr=lmii()
    ANS=[]
    ans=sum(sorted(arr[:M])[:K])
    ANS.append(ans)
    mu =  SortedMultiset(sorted(arr[:M]))
    ...
    for i in range(N-M):
        ...
        if v==0 and mu[K-1]>arr[i+M]:
            ans-=mu[K-1]
            ans+=arr[i+M]
        elif v==1 and mu[K-1]>arr[i+M]:
            ans+=arr[i+M]
        elif v==1:
            ans+=mu[K-1]
        mu.add(arr[i+M])
